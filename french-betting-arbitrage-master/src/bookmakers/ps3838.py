...
def get_page(competition):
    ...
    driver.get(url['url'])
    driver.find_element(By.CSS_SELECTOR, f"[data-league='{url['code']}']").click()
    found = 0
    while (found >= 0):
        try:
...
def get_games(competition):
    ...
    while not good:
        try:
            dates = driver.find_elements(By.CSS_SELECTOR, ".dateMenutb td:not([data-date='null'])")
    ...
    for date in dates:
        ...
        while not good:
            try:
                date_new = driver.find_element(By.LINK_TEXT, date)
                date_new.click()
